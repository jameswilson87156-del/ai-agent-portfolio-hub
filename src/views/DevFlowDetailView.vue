  ...
  <div class="mcp-detail-page devflow-detail-page">
    ...
    <main class="detail-main">
      <DetailHero
        ...
        dossier-stage="P3B / LOCAL DEMO"
        dossier-description="以真实本地浏览器截图、README 记录、构建验收和明确边界说明构成证据。页面不代表生产级 AI IDE。"
      />
      ...
      <section class="detail-section problem-section" aria-labelledby="problem-title">
        <header class="detail-section-heading">
          ...
          <p>
            DevFlow 关注 AI Coding workflow，而不是 Tool Governance。它把一次生成行为拆成可记录、可保存、可审核的工程流程。
          </p>
        </header>
      ...
      <section class="detail-section module-section" aria-labelledby="modules-title">
        <header class="detail-section-heading">
          ...
          <p>
            页面不是普通 dashboard，而是把 AI Coding 工作流拆成可演示的产品模块：输入、生成、引用、记录、审核和复盘。
          </p>
        </header>
      ...
      <EngineeringEvidenceGrid
        ...
        description="证据来自 DevFlow 源项目 README、docs 与真实浏览器截图；Provider、Knowledge Base 和 Human Review 能力均保持 demo 边界。"
      />
      <BoundaryPanel :sections="devFlowDetail.trustPanel" />
      <InterviewNotes :items="devFlowDetail.interviewQuestions" />

      <footer class="detail-footer">
        <p>Wang Zhenlong · DevFlow Copilot · Portfolio case study</p>
        <div>
          <RouterLink to="/projects">← Projects</RouterLink>
          <a :href="devFlowDetail.githubUrl" target="_blank" rel="noreferrer">GitHub ↗</a>
        </div>
      </footer>
    </main>
  </div>
